...
/// // при первом запуске — передать токен из СВОЕГО MMP-SDK (POC-1) ИЛИ nil.
/// // Токен лежит в `deep_link_sub1` (дубль — `af_sub1`), НЕ в `deep_link_value`:
/// // там у нас константа `handoff`. См. `AttributionResolver`.
/// Web2App.identify(deepLinkValue: afDeepLinkSub1) { result in ... }
...
public enum Web2App {
    ...
    /// всегда при запросе. Отказ молчаливый: сервер отвечает «успех» в любом случае (защита
    /// от перебора плательщиков), поэтому по ответу проблему не увидеть.
    /// Не используйте наш guid как profile-id провайдера (`Purchases.logIn(guid)` и т.п.) —
    /// с таким значением дозапись второй платформы через страницу не сработает никогда.
    /// Заменить сохранённое значение можно только серверной ручкой
    /// `POST /s2s/v1/identity/link-profile` (Bearer `sk_`, право `identity:write`).
    ///
    ...
    /// платформы (берётся из её SDK ДО показа страницы). SDK дописывает их в URL страницы;
    /// связку guid ↔ profile-id пишет страница, но ТОЛЬКО в пустой слот: занятое значение
    /// через страницу не перезаписывается, а вторую платформу можно дописать, лишь предъявив
    /// в этом же открытии верные значения всех уже занятых слотов. Отсюда рецепт: передавайте
    /// ОБА идентификатора при КАЖДОМ открытии. Отказ молчаливый — сервер всегда отвечает
    /// «успех». Наш guid как profile-id провайдера не годится (тогда дозапись второй
    /// платформы через страницу не сработает никогда); замена сохранённого значения — только
    /// серверной ручкой `POST /s2s/v1/identity/link-profile`.
    ...
    /// наш guid в роли profile-id провайдера не годится. Подробнее — `openWebPaywallEmbedded`.
    ///
